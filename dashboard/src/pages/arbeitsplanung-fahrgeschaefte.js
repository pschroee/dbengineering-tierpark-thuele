...
const ArbeitsplanungFahrgeschäfte = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [initialData, setInitialData] = useState(null);
  const [columns, setColumns] = useState([]);

  const ENTITY = "MitarbeiterFahrgeschäft";
  const PRIMARY_KEY = "Id";
  const TITLE = "Arbeitsplanung Fahrgeschäfte";
  ...
  const handleCreate = async (item) => {
    try {
      ...
      const arbeitsanfang = new Date(result.data["Arbeitsanfang"]).getTime() / 1000;
      const arbeitsende = new Date(result.data["Arbeitsende"]).getTime() / 1000;
      result.data["Arbeitszeit"] = arbeitsende - arbeitsanfang;
      setIsLoading(false);
      return result.data;
    } catch (e) {
      setIsLoading(false);
...
ArbeitsplanungFahrgeschäfte.getLayout = (page) => <Layout>{page}</Layout>;

export default ArbeitsplanungFahrgeschäfte;

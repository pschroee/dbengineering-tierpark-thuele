...
const Rollenberichtigungen = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [initialData, setInitialData] = useState(null);
  const [columns, setColumns] = useState([]);

  const ENTITY = "Rollenberechtigung";
  const PRIMARY_KEY = "Id";
  const TITLE = "Rollenberechtigung";
...
Rollenberichtigungen.getLayout = (page) => <Layout>{page}</Layout>;

export default Rollenberichtigungen;
